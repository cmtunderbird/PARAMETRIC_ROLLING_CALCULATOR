...
// ─── Main App ─────────────────────────────────────────────────────────────────
// rollDamping (ζ): non-dimensional damping ratio.
// Without bilge keels: 0.03–0.06 | With bilge keels: 0.07–0.12 | With active stabilisers: 0.12–0.20
// This is the single most consequential parameter — at resonance roll amplitude ∝ 1/(2ζ).
const PRESETS = {
...
export default function ParametricRollingCalculator() {
  ...
  return (
    <div style={{ fontFamily: "'JetBrains Mono', 'Fira Code', monospace", background: "linear-gradient(135deg, #0B1120 0%, #0F172A 50%, #111827 100%)", color: "#E2E8F0", minHeight: "100vh", padding: 0 }}>
      ...
      <div style={{ padding: "8px 24px 0", display: "flex", gap: 4, flexWrap: "wrap" }}>{tabBtn("dashboard", "Dashboard")}{tabBtn("vessel", "Vessel Config")}{tabBtn("weather", "Weather Sources")}{tabBtn("polar", "Polar Analysis")}{tabBtn("route", "Route Chart")}{tabBtn("reference", "Reference")}</div>
      <div style={{ padding: "16px 24px" }}>
        ...
        {activeTab === "vessel" && (<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, maxWidth: 800 }}>
          {panel(<>{sectionHeader("Hull Dimensions")}
          ...
          {panel(<>{sectionHeader("Stability & Damping")}
            <Field label="Metacentric Height (GM)" value={ship.GM} onChange={(v) => updateShip("GM", v)} unit="m" step={0.05} min={0.01} />
            <Field label="Roll Damping Ratio (ζ)" value={ship.rollDamping ?? 0.05} onChange={(v) => updateShip("rollDamping", Math.max(0.01, Math.min(0.30, v)))} unit="" step={0.005} min={0.01} max={0.30} />
            <div style={{ marginBottom: 12, padding: "6px 8px", background: "#0F172A", borderRadius: 4, border: "1px solid #334155", fontSize: 9, color: "#64748B", lineHeight: 1.8, fontFamily:"'JetBrains Mono',monospace" }}>
